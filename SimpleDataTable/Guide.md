...
1. Change colons to dashes in the commented code; remove most double quotation marks (except for inline string values like `"Id"` for `keyField` value) and standard provider references `"!v"` and `"!c"` from the commented code.
...
1. Add code to `SimpleAppUsesLWC` to display an error because of invalid object name.
```
...
```
2. Notice two null-pointer errors.
3. Change code of `simpleTable.js` to transform string value into array and fix one of the null pointer error
```	
    connectedCallback() {
        getColumnsAndData({
            sObjectName: this.sObjectName,
...
```
4. Use Javascript Debugging in case if you don't see error displayed
5. Notice null-pointer error and learn how to transform it into a meaningful exception by inserting a piece of code to `SchemaProvider` class before line #6.
```
            if ( Type.forName(token) == null ) {
                throw new CustomException(token + ' is not valid SObject name');
            }
```
and introduce an inner `CustomException` class in the bottom of the class `public class CustomException extends Exception {}`
6. Comment catch block in `SimpleDataTableController` lines 41, 43-45 and learn that unhandled exceptions result into non-readable errors. Notice non-readable exception and then revert changed in `SimpleDataTableController`.

## Fix sorting by converting `updateColumnSorting` method into Javascript module method
1. Notice that sorting is not working in the current LWC version but that it is working in the Aura version.
2. Copy methods `updateColumnSorting`, `sortData` and `sortBy` from `SimpleDataTableHelper.js` into `simpleTable.js` and comment them out.
3. Remove `function` keyword and colon from `sortData` and `sortBy` function definitions.
4. Replace `event` by `e`, replace `getSource()` by `srcElement`, remove `set("v.` and corresponding closing parenthesis and replace comma by assignment operator.
5. Replace `getParam('` by `detail.` and remove corresponding closing parenthesis.
6. Replace `helper` by `this` and replace `cmp` param by `e.srcElement`.
7. Uncomment `sortBy` code, it contains valid LWC code and doesn't have to be converted.
8. Inside `sortData`, replace `cmp` with `src`, and replace `cmp.get("v.data")` with `JSON.parse(JSON.stringify(src.data))` and `cmp.set("v.data",` with `src.data = ` and remove corresponding closing parenthesis.
9. Deploy changes and confirm that sorting now works in LWC version.
...
Not shown in the workshop video: additional task for reader
